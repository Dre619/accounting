...
function onSearch() {
    ...
    debounce = setTimeout(() => {
        router.get('/sales-orders', { status: props.currentStatus, search: searchVal.value }, { preserveState: true, replace: true });
    }, 350);
}
    ...
    <AppLayout>
        <div class="flex flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold">Sales Orders &amp; Quotes</h1>
                <Button as-child>
                    <Link href="/sales-orders/create"><Plus class="mr-2 h-4 w-4" /> New Order</Link>
                </Button>
            </div>
            ...
            <div class="flex gap-2 flex-wrap">
                <Button
                    ...
                    size="sm"
                    @click="router.get('/sales-orders', { status: tab.key, search: searchVal })"
                >
                    {{ tab.label }}
            ...
            <Card>
                <CardContent class="pt-4">
                    <div class="relative mb-4 max-w-sm">
                        ...
                        <Input v-model="searchVal" placeholder="Search by number or customer…" class="pl-9" @input="onSearch" />
                    </div>
                        ...
                        <TableBody>
                            <TableRow v-for="o in props.orders.data" :key="o.id">
                                <TableCell class="font-medium">
                                    <Link :href="`/sales-orders/${o.id}`" class="hover:underline">{{ o.order_number }}</Link>
                                </TableCell>
                                ...
                                <TableCell class="text-muted-foreground">{{ o.valid_until ?? '—' }}</TableCell>
                                <TableCell class="text-right font-medium">ZMW {{ fmt(o.total) }}</TableCell>
                            </TableRow>
                            <TableRow v-if="!props.orders.data?.length">
                                <TableCell colspan="6" class="py-10 text-center text-muted-foreground">
                                    No sales orders found.
                                    <Link href="/sales-orders/create" class="ml-1 underline">Create one</Link>
                                </TableCell>
                            </TableRow>
                        </TableBody>
                    </Table>
